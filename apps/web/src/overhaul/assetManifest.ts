...
export const ASSET_MANIFEST: AssetManifestEntry[] = [
  ...
  // ---- paths/water/fences (декоративный периметр) -------------------------
  {
    ...
    source: { kind: 'procedural', gen: 'generateThicketTile' },
    purpose: 'Заросли за границей открытого сектора (см. worldConfig.SECTOR/CAMERA_BOUNDS) — непроходимо',
    sizePx: [32, 32],
    frameCount: 1,
    anchor: [0, 0],
    collisionBox: [0, 0, 32, 32],
    ...
    sortLayer: 'ground',
    status: 'temporary',
    note: 'Тёмный/плотный вариант травы — читается как "дикая территория, сюда пока нельзя", не притворяется обычным газоном.',
  },
  {
    id: 'fence_gate',
    group: 'paths_water_fences',
