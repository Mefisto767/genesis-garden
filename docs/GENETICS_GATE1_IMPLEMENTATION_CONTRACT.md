...
`GameStore.breedNurseryV2()` определяет `isTutorialBreed = seedParent.tutorialStarter===true && pollenParent.tutorialStarter===true && geneticsTutorialBreedsCompleted<2` ДО расчёта стоимости и передаёт в `breedV2` либо `mulberry32(tutorialBreedRngSeed(geneticsTutorialBreedsCompleted))`, либо обычный `this.rng`. Помимо подмены RNG, `isTutorialBreed` также обнуляет стоимость: `cost = isTutorialBreed || !firstBreedFreeClaimed ? 0 : breedCostV2(...)` — **оба** обучающих скрещивания безусловно бесплатны, не только первое (уточнение этого прохода реализации: onboarding spec §4.2 требует, чтобы второй урок про скрытый признак ГАРАНТИРОВАННО состоялся сразу после первого — если бы стоимость второго скрещивания считалась по обычному прайсу (8 пыльцы одновидового скрещивания), а игрок к этому моменту успел накопить меньше через обычный сбор урожая (по таймлайну §1 — 4 пыльцы к моменту первого скрещивания), гарантия §4.2 была бы нарушена самим экономическим гейтом). Валидация видов и структура атомарного обновления (§4.14.13) при этом идентичны для tutorial и обычного скрещивания — различаются только RNG и стоимость. Третье и последующие скрещивания той же пары (`geneticsTutorialBreedsCompleted===2`), любое скрещивание с не-tutorial-родителем и любой ветеранский save (у которого `tutorialStarter` никогда не был выставлен) — всегда обычный `this.rng` и обычная стоимость, без исключений.
