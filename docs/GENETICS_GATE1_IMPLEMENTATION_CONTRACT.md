...
**Recalibrated by Slice 13 (проход 16, §4.15.3)** — до этого прохода «Вес появления» существовал только как документация, ни разу не закодированный ни в каком исполняемом файле репозитория (в игре нет механики «случайного рождения генома» — эта колонка описывает исключительно генератор популяции статистического теста rarity-калибровки). Slice 13 впервые превратил эту колонку в исполняемый код (`apps/web/src/game/rarityCalibrationV2.ts`) и слегка её откалибровал вместе с rarity points (ниже) — значения обоих столбцов ниже отражают финальные, откалиброванные числа, не исходные из проходов 5-6. Rank-1 («доминантный/самый частый») вес каждого локуса поднят примерно на 6% (пример: `stem_standard` 60→64) за счёт ничего (веса остальных аллелей локуса оставлены как были — сумма веса локуса просто выросла, что нормально для `weightedPick` — относительные пропорции важны, не абсолютная сумма) — сдвигает распределение симулированной популяции немного в сторону более однородных (более common) геномов, компенсируя избыточную концентрацию в Uncommon/Rare, которую даёт калибровка rarity points ниже. Полная методология и итоговое измеренное распределение — §4.15.2/§4.15.3.
...
|---|---|---|---:|---:|---:|---|---|---|---|---|
| `stem_standard` | Прямой | Прямой | 1 | 64 | 0 | да | да | да | нет | да |
...
| Аллель ID | Название | Визуальное выражение (временное) | Dominance rank | Вес появления | Rarity points | Солнечник | Колокольник | Межвидовое наследование | Mutation pool | Neutral migration ID |
|---|---|---|---:|---:|---:|---|---|---|---|---|
| `leaf_standard` | Обычный | Обычный | 1 | 53 | 0 | да | да | да | нет | да |
| `leaf_broad` | Широкий | Широкий | 2 | 25 | 1 | да | да | да | нет | — |
| `leaf_narrow` | Игольчатый | Игольчатый | 3 | 17 | 3 | да | да | да | нет | — |
...
| Аллель ID | Название | Визуальное выражение (временное) | Dominance rank | Вес появления | Rarity points | Солнечник | Колокольник | Межвидовое наследование | Mutation pool | Neutral migration ID |
|---|---|---|---:|---:|---:|---|---|---|---|---|
| `flower_standard` | Классический | Классический | 1 | 53 | 0 | да | да | да | нет | да |
| `flower_fan` | Веерный | Веерный | 2 | 25 | 1 | да | да | да | нет | — |
| `flower_cap` | Шляпковидный | Шляпковидный | 3 | 17 | 3 | да | да | да | нет | — |
...
| Аллель ID | Название | Визуальное выражение (временное) | Dominance rank | Вес появления | Rarity points | Солнечник | Колокольник | Межвидовое наследование | Mutation pool | Neutral migration ID |
|---|---|---|---:|---:|---:|---|---|---|---|---|
| `primary_honey` | Медовый | Медовый (#FFC85C) | 1 | 17 | 0 | да | да | да | нет | — |
| `primary_amber` | Янтарный | Янтарный (#F5A623) | 2 | 19 | 0 | да | да | да | нет | — |
| `primary_sunset` | Закатный | Закатный (#FF6F59) | 3 | 21 | 0 | да | да | да | нет | — |
| `primary_coral` | Коралловый | Коралловый (#FF8C77) | 4 | 23 | 0 | да | да | да | нет | — |
| `primary_lilac` | Лиловый | Лиловый (#CFA1E8) | 5 | 8 | 1 | да | да | да | нет | — |
| `primary_violet` | Фиолетовый | Фиолетовый (#B678D9) | 6 | 10 | 1 | да | да | да | нет | — |
| `primary_leaf` | Листовой зелёный | Листовой зелёный (#89D65C) | 7 | 5 | 3 | да | да | да | нет | — |
...
| Аллель ID | Название | Визуальное выражение (временное) | Dominance rank | Вес появления | Rarity points | Солнечник | Колокольник | Межвидовое наследование | Mutation pool | Neutral migration ID |
|---|---|---|---:|---:|---:|---|---|---|---|---|
| `secondary_forest` | Лесной | Лесной (#57993A) | 1 | 19 | 0 | да | да | да | нет | — |
| `secondary_sunset` | Закатный | Закатный (#FF6F59) | 2 | 21 | 0 | да | да | да | нет | — |
| `secondary_amber` | Янтарный | Янтарный (#F5A623) | 3 | 23 | 0 | да | да | да | нет | — |
...
| `secondary_sky` | Небесный | Небесный (#A9D4E2) | 6 | 8 | 3 | да | да | да | нет | — |
...
| Аллель ID | Название | Визуальное выражение (временное) | Dominance rank | Вес появления | Rarity points | Солнечник | Колокольник | Межвидовое наследование | Mutation pool | Neutral migration ID |
|---|---|---|---:|---:|---:|---|---|---|---|---|
| `leaf_color_meadow` | Луговой | Луговой (#89D65C) | 1 | 58 | 0 | да | да | да | нет | — |
| `leaf_color_fresh` | Свежая зелень | Свежая зелень (#6FBE44) | 2 | 30 | 1 | да | да | да | нет | — |
| `leaf_color_forest` | Тёмный лесной | Тёмный лесной (#57993A) | 3 | 15 | 3 | да | да | да | нет | — |
...
| Аллель ID | Название | Визуальное выражение (временное) | Dominance rank | Вес появления | Rarity points | Солнечник | Колокольник | Межвидовое наследование | Mutation pool | Neutral migration ID |
|---|---|---|---:|---:|---:|---|---|---|---|---|
| `pattern_solid` | Однотонный | Однотонный (= legacy `solid`) | 1 | 48 | 0 | да | да | да | нет | да |
| `pattern_duotone` | Двухцветный | Двухцветный (= legacy `duotone`) | 2 | 30 | 1 | да | да | да | нет | — |
| `pattern_spots` | Пятнистый | Пятнистый | 3 | 14 | 3 | да | да | да | нет | — |
| `pattern_stripes` | Полосатый | Полосатый | 4 | 9 | 4 | да | да | да | нет | — |
...
| Аллель ID | Название | Визуальное выражение (временное) | Dominance rank | Вес появления | Rarity points | Солнечник | Колокольник | Межвидовое наследование | Mutation pool | Neutral migration ID |
|---|---|---|---:|---:|---:|---|---|---|---|---|
| `size_normal` | Обычный | Обычный | 1 | 53 | 0 | да | да | да | нет | — |
...
| Аллель ID | Название | Визуальное выражение (временное) | Dominance rank | Вес появления | Rarity points | Солнечник | Колокольник | Межвидовое наследование | Mutation pool | Neutral migration ID |
|---|---|---|---:|---:|---:|---|---|---|---|---|
| `aura_none` | Без эффекта | Без эффекта | 1 | 58 | 0 | да | да | да | нет | — |
| `aura_faint` | Пыльцевая дымка | Пыльцевая дымка | 2 | 28 | 1 | да | да | да | нет | — |
| `aura_glow` | Мягкое свечение | Мягкое свечение | 3 | 15 | 3 | да | да | да | нет | — |
...
**Recalibrated by Slice 13 (проход 16, §4.15.3)** — `Common`/`Uncommon`/`Rare` boundaries (`3`, `6`, `8`) are UNCHANGED (pinned by the two guaranteed tutorial breeds, §4.6.3/§4.6.4: naturalScore 4 and 5 must both land in `Uncommon`); only the Epic/Legendary boundary moved, `10` → `11`:
...
## 4.15. Финальный объединённый пакет Gate 1 (проход 16) — carryover fix, Slice 13 rarity calibration, Slice 14 финальный E2E

Владелец принял Slice 12 fix-pass (`bc26088e3822210c8fb11dd018414bf86291d385`, §4.14.14) **полностью** и заказал финальный объединённый пакет: один небольшой carryover data-correctness fix, найденный финальным аудитом Slice 12; Slice 13 (rarity calibration на 100 000 измеряемых результатов); Slice 14 (финальный сквозной Gate 1 E2E + regression всех четырёх комбинаций флагов). **После этого пакета Genetics Gate 1 заканчивается** — Slice 15, Lab L3/L4, `GeneLock`, новые виды, социальные механики, Supabase/Paddle/NFT, art-pass не начинаются.

Ровно три последовательных коммита (без amend/rebase): `docs(genetics): lock final Gate 1 calibration and regression` (этот коммит), `test(genetics): calibrate V2 rarity distribution` (carryover fix + его unit/store-тесты + Slice 13 калибровка — оба вместе, по прямому указанию владельца), `test(genetics): add final Gate 1 E2E regression` (Slice 14).

### 4.15.1. Carryover fix — `HybridSeedV2.parentSpeciesIds`

**Дефект, найденный финальным аудитом Slice 12** (не новая развилка — исправление существующего поведения `revealParentSpecies`, §4.14.14): межвидовой breed species1×species2 создаёт `HybridSeedV2`; до созревания один из родителей перерабатывается (`recycleSpecimenV2`); при созревании `harvestHybridV2` вычисляет `revealParentSpecies` поиском ЖИВЫХ родителей в `this.state.specimens` по `hybrid.parentIds` — для отсутствующего (переработанного) родителя этот поиск подставлял `hybrid.genomeV2.speciesId` (вид РЕБЁНКА, который всегда равен виду Seed Parent, §4.12) вместо честного «неизвестно». Если переработан именно Pollen Parent межвидовой пары, оба элемента `revealParentSpecies` становились равны виду Seed Parent — Reveal ошибочно считал пару одновидовой и показывал `От первого/второго растения` вместо корректных `← Солнечник`/`← Колокольник` (`traitOriginLabelsV2`, revealV2.ts).

**Исправление**: новое опциональное поле `HybridSeedV2.parentSpeciesIds?: [number, number]` (`geneticsV2.ts`) — заполняется в `breedNurseryV2` (`store.ts`) непосредственно из `[seedParent.genomeV2.speciesId, pollenParent.genomeV2.speciesId]` **в момент успешного скрещивания**, пока оба родителя гарантированно существуют. Поле — часть `HybridSeedV2`, поэтому проходит без изменений через `nurseryTray`, посадку (`plantHybridSeedV2` копирует объект целиком, не пересобирает его) и `Plot.hybridV2.phase==='growing'`, включая полный JSON save/reload round-trip (обычный массив из двух чисел, не теряется сериализацией). При созревании (`harvestHybridV2`) `revealParentSpecies` теперь строится как `hybrid.parentSpeciesIds ?? [живой-поиск-как-раньше]` — предпочитает захваченное на breed-time значение, безопасно откатывается на прежний live-parent lookup только для `HybridSeedV2`, сериализованного ДО этого фикса (поле физически отсутствует в старых данных — `undefined`, не `null`, обычная опциональность TypeScript/JSON). Текущий live-parent lookup (для родителей, ещё присутствующих в `state.specimens`) продолжает использоваться без изменений для естественного раскрытия (`computeNaturalRevealsV2`) — этот carryover fix касается ТОЛЬКО отображаемых species-меток `revealParentSpecies`, не natural reveal, который и раньше безопасно пропускал отсутствующую сторону (revealV2.ts, `seedGenomeV2`/`pollenGenomeV2` независимо nullable).

`SAVE_VERSION` не меняется (остаётся `4`) — оба новых поля аддитивны, опциональны, безопасны для старых V4-save.

**Обязательный regression-тест** (реализован в `store.revealLifecycleV2.test.ts`, коммит 2): межвидовой breed species1×species2 через реальный `breedNurseryV2`; Pollen Parent (Колокольник) переработан через `recycleSpecimenV2` ДО посадки/созревания; ребёнок сажается и созревает через реальный `harvestHybridV2`; `Specimen.revealParentSpecies` всё равно равен `[1, 2]`; `traitOriginLabelsV2` для seed/pollen сторон даёт ровно `← Солнечник`/`← Колокольник`, никогда `От первого/второго растения`; ни один рендер не содержит raw numeric species ID; natural reveal безопасно пропускает отсутствующую (переработанную) сторону, не падает. Плюс отдельный тест на безопасный fallback: `HybridSeedV2` без `parentSpeciesIds` (симулирует pre-fix save) при обоих живых родителях воспроизводит ровно прежнее (live-lookup) поведение.

### 4.15.2. Slice 13 — методология rarity-калибровки на 100 000 измеряемых результатов

Заменяет docs-only Python-симуляцию §4.5.5 РЕАЛЬНОЙ (исполняемой, часть репозитория) симуляцией — `apps/web/src/game/rarityCalibrationV2.ts` (test-support модуль, физически не импортируется ни из `store.ts`, ни из UI/Phaser-кода — не попадает в production-бандл тем же tree-shaking, каким исключается любой другой неиспользуемый модуль) + `apps/web/src/game/rarityCalibrationV2.test.ts`. Методология зафиксирована здесь **до** того, как какая-либо production-константа (`rarityV2.ts`) была изменена этим проходом — калибровка (§4.15.3) выполнялась ПОСЛЕ фиксации этой методологии и интервалов §4.15.4, не наоборот.

**Алгоритм** (`runRarityCalibrationSimulationV2`, детерминирован от одного `RARITY_CALIBRATION_SEED = 20260829`, единый непрерывный поток `mulberry32`):

1. **Подготовительная популяция (burn-in, НЕ входит в измеряемую выборку).** 4000 базовых геномов: каждый локус — независимый взвешенный случайный выбор аллеля по «весу появления» (§4.2, аллели с весом 0 — `aura_radiant` — исключены из пула, как и в docs-only §4.5.5); `speciesId` — ровно 2000 генома species1, 2000 species2, по чётности индекса (гарантированный, не вероятностный 50/50 сплит — «популяция содержит species 1 и 2» соблюдается конструктивно). Затем три поколения по 2000 скрещиваний случайных пар ИЗ ТЕКУЩЕЙ популяции (население растёт 4000→6000→8000→10000, никогда не заменяется) через РЕАЛЬНЫЙ `breedV2` — включая одновидовые (species1×1, species2×2) и межвидовые (1×2, 2×1) пары, поскольку пары выбираются случайно из смешанной по видам популяции. Результаты этих 6000 скрещиваний нигде не подсчитываются по тирам — используются только итоговые геномы (в т.ч. потомки с реально мутировавшим `aura_radiant`, как в настоящей коллекции игрока после многих скрещиваний).
2. **Измерение — ровно 100 000 успешных `breedV2`.** Каждая пара — два РАЗЛИЧНЫХ случайных индекса в финальной популяции (population НЕ уменьшается между вызовами — тот же принцип, что стабильная коллекция, повторно скрещиваемая много раз). `pityCounter` — ОДИН непрерывный счётчик через ВЕСЬ прогон (burn-in И измерение — один RNG-поток от одного seed, без сброса). Species 1/2-only популяция никогда не даёт `unsupported_species` — все 100 000 попыток успешны по построению; цикл тем не менее defensively повторяет попытку при гипотетическом отказе, чтобы «ровно 100 000 успешных» было гарантией конструкции, а не допущением.
3. Для каждого измеренного результата — подсчитывается один из шести тиров `result.rarity` (реальный `rarityOfV2`, вызванный изнутри реального `breedV2`, не изолированно).

**Родители разнообразны по построению**: каждый геном популяции — независимый взвешенный случайный выбор по 9 локусам (не один фиксированный генотип, повторяемый много раз) плюс дополнительное разнообразие от трёх поколений реального наследования/мутации.

**Побайтовая воспроизводимость**: один числовой seed, без `Math.random`/wall-clock/недетерминированного порядка итерации — тест `rarityCalibrationV2.test.ts` включает отдельную проверку «два запуска с тем же seed дают идентичный результат».

### 4.15.3. Slice 13 — измеренное (до калибровки) распределение и решение о необходимости калибровки

**Первый прогон — БЕЗ каких-либо изменений production-констант** (исходные `rarityV2.ts` пороги/points из Slice 3, впервые закодированные веса §4.2 взяты as-is из документации):

| Tier | Count | % | Целевой % | Интервал §4.15.4 | В интервале? |
|---|---:|---:|---:|---|---|
| Common | 51 080 | 51.080% | 50.0% | [49050, 50950] | НЕТ |
| Uncommon | 31 488 | 31.488% | 30.0% | [29125, 30875] | НЕТ |
| Rare | 12 287 | 12.287% | 14.0% | [13340, 14660] | НЕТ |
| Epic | 4 232 | 4.232% | 5.0% | [4585, 5415] | НЕТ |
| Legendary | 753 | 0.753% | 0.9% | [720, 1080] | да |
| Mythic | 160 | 0.160% | 0.1% | [40, 160] | да (на границе) |

4 из 6 тиров вне заранее зафиксированных интервалов (§4.15.4) — калибровка обязательна по собственному правилу этого документа («если не входит в интервалы — откалибровать минимально необходимое»).

**Калибровка** — изменены ТОЛЬКО rarity points/thresholds/weights (никогда dominance/allele catalogs/mutation pity curve/mutation tier floors/RNG call order/inheritance/новый rarity-roll — `rarityOfV2` остаётся чистой детерминированной функцией `genome+mutationId`):

1. **Rarity points** (`rarityV2.ts`) — семь значений подняты с 2 на 3 (`leaf_narrow`, `flower_cap`, `primary_leaf`, `secondary_sky`, `leaf_color_forest`, `pattern_spots`, `aura_glow`), одно с 3 на 4 (`pattern_stripes`). Выбраны специально так, чтобы НИ ОДНО из пяти значений, от которых зависят оба гарантированных обучающих скрещивания (`leaf_broad`, `flower_fan`, `leaf_color_fresh`, `aura_faint`, `size_large` — все rank-2, все `=1`), не изменилось.
2. **Threshold** (`tierFromNaturalScore`) — только граница Epic/Legendary: `10`→`11`. Common/Uncommon (`3`)/Uncommon/Rare (`6`)/Rare/Epic (`8`) НЕ трогались — Uncommon/Rare граница `6` заперта требованием «второе обучающее скрещивание (score 5) остаётся Uncommon», Common/Uncommon граница `3` — требованием «первое обучающее скрещивание (score 4) остаётся Uncommon».
3. **`MYTHIC_CO_THRESHOLD`** — `5`→`6`.
4. **Weights** (`rarityCalibrationV2.ts`, впервые закодированные этим же проходом — §4.2 примечание) — rank-1 (самый частый/доминантный) вес каждого локуса поднят примерно на 6% относительно остальных весов того же локуса (полный список — таблицы §4.2 выше).

Пункты 1-3 калибруют РЕАЛЬНУЮ игровую формулу `rarityOfV2` — применяются к каждому скрещиванию каждого игрока. Пункт 4 калибрует ТОЛЬКО генератор популяции этого теста (в игре нет механики «случайного рождения генома» — см. примечание §4.2) — не влияет ни на один игровой расчёт.

**Итоговое (откалиброванное) распределение**, измеренное после калибровки — окончательные числа, воспроизводимые побайтово `rarityCalibrationV2.test.ts`:

| Tier | Count | % | Целевой % | Отклонение (пп) | Интервал §4.15.4 | σ от цели |
|---|---:|---:|---:|---:|---|---:|
| Common | 50 528 | 50.528% | 50.0% | +0.528 | [49050, 50950] | 3.34 |
| Uncommon | 29 648 | 29.648% | 30.0% | −0.352 | [29125, 30875] | 2.41 |
| Rare | 13 700 | 13.700% | 14.0% | −0.300 | [13340, 14660] | 2.73 |
| Epic | 5 230 | 5.230% | 5.0% | +0.230 | [4585, 5415] | 3.34 |
| Legendary | 774 | 0.774% | 0.9% | −0.126 | [720, 1080] | 4.22 |
| Mythic | 120 | 0.120% | 0.1% | +0.020 | [40, 160] | 2.00 |
| **Итого** | **100 000** | **100.000%** | **100.0%** | — | — | — |

Все шесть тиров внутри пре-зафиксированных интервалов; ни один σ не превышает 6 (интервалы построены как ±6σ, §4.15.4) — не reverse-engineered под этот конкретный результат: то же интервальное правило было записано в docs-lock коммите ДО того, как какая-либо production-константа была изменена (§4.15.2/§4.15.4).

**Обязательный пост-калибровочный regression** (проверено, все проходят на РЕАЛЬНОМ `breedV2`, не изолированным вызовом `rarityOfV2`):
- Первое обучающее скрещивание (contract §4.6.3) — score 4, остаётся ровно Uncommon.
- Второе обучающее скрещивание (contract §4.6.4) — score 5, остаётся ровно Uncommon, естественное раскрытие `size_large` не затронуто (calibration не касалась dominance/inheritance).
- `pollenRewardV2`/`RARITY_RECYCLE_DUST` (`pollenV2.ts`/`recyclingV2.ts`) — оба индексируются по ИМЕНИ тира (`Record<RarityTierV2, number>`), не по числовому score — калибровка порогов/points их не затрагивает структурно; таблицы значений не менялись.
- Mythic по-прежнему достижим ТОЛЬКО через `mutationTier==='Signature' && naturalScore>=MYTHIC_CO_THRESHOLD` — правило не менялось, изменилось только числовое значение порога (§4.5.4).
- Все 887 прежних unit/rarity/store-тестов сохранены; 6 тестов с захардкоженными граничными score-фикстурами (`rarityV2.test.ts` ×3, `mutationV2.test.ts` ×1, `recyclingV2.test.ts` ×2) обновлены на корректные для НОВЫХ порогов фикстуры — не ослаблены, те же самые границы (нижняя/верхняя Rare, нижняя/верхняя Epic, Legendary, Mythic co-threshold) проверяются на новых, а не старых числах.

### 4.15.4. Статистические интервалы (зафиксированы ДО калибровки, §4.15.3)

Не единый процент допуска для всех тиров — отдельный count-интервал на каждый из шести, учитывающий ожидаемую биномиальную дисперсию именно ЭТОГО тира при `n=100000`: `σ = √(n·p·(1-p))` для целевой доли `p`, интервал `[n·p − 6σ, n·p + 6σ]`, округлён наружу до ближайших 10. Одно и то же число стандартных отклонений (`6`) применяется ко всем шести тирам — именно поэтому интервал Mythic (ожидание ~100 из 100000, `σ≈10`, интервал `[40,160]`, ширина ±60%) намного ýже в ПРОЦЕНТНОМ выражении интервала Common (ожидание 50000, `σ≈158`, интервал `[49050,50950]`, ширина ±1.9%), но соразмерен ему в единицах σ — Mythic не получает допуск Common (обратное было бы бессмысленным: допуск Common в абсолютных числах ~±950, что для Mythic перекрыло бы вообще весь диапазон 0-1050, тест перестал бы что-либо проверять).

| Tier | Целевой % | n·p | σ | Интервал (±6σ, округлено) |
|---|---:|---:|---:|---|
| Common | 50.0% | 50 000 | 158.11 | [49050, 50950] |
| Uncommon | 30.0% | 30 000 | 145.77 | [29125, 30875] |
| Rare | 14.0% | 14 000 | 109.98 | [13340, 14660] |
| Epic | 5.0% | 5 000 | 68.92 | [4585, 5415] |
| Legendary | 0.9% | 900 | 29.87 | [720, 1080] |
| Mythic | 0.1% | 100 | 9.995 | [40, 160] |

Тест выводит фактические counts и percentages ВСЕХ шести тиров при падении (не только тир, вышедший за интервал) — `rarityCalibrationV2.test.ts`.

### 4.15.5. Slice 14 — финальный Gate 1 E2E (`test-e2e-genetics-v2-gate1.mjs`)

Один новый канонический сценарий, полный настоящий пользовательский путь (37 пронумерованных проверок — полный список воспроизведён в отчёте владельцу этого прохода, не дублируется здесь построчно, чтобы не расходиться при точечных правках сценария): новый save → контекстный Genetics intro (не старый 4-слайдовый онбординг) → выбор двух tutorial-родителей → первое бесплатное скрещивание → фенотип неизвестен до созревания (переживает reload во время роста) → Nursery Tray → посадка → первый Reveal (Солнечник/Uncommon/9 признаков/происхождение/без мутации, acknowledgement переживает reload) → Lab L2/Колокольник открываются наградой → второе обучающее скрещивание (8 пыльцы, точная ошибка при нехватке, 0 RNG/no-op на отказе, ровно −8 при успехе) → второй offspring раскрывает `size_large` естественно, `source:'natural'` у обоих родителей → повторный harvest не дублирует Specimen/Reveal → переработка Nursery Seed и mature Specimen (favorite защищён) → микроскоп (ровно 3 пыли, раскрытый locus повторно недоступен) → межвидовое скрещивание 1×2 и 2×1 (species ребёнка = Seed Parent, родословная без raw ID, удалённый до maturity межвидовой родитель не ломает species-origin labels — прямая E2E-проверка carryover fix §4.15.1 через реальный UI, не только unit/store) → Nursery Tray 8/8 блокирует девятый breed → Ботаническая книга (5 рабочих разделов + 1 «Скоро») → tutorial replay не меняет сериализованное состояние → Lumi максимум одна подсказка → mobile viewport без horizontal overflow → 0 console/page errors.

Существующие Playwright-файлы (`test-e2e-genetics-v2*.mjs` вся семья, включая `-slice8`/`-slice9`/`-slice10-11`/фокусированный Slice 12) НЕ удалены и не ослаблены — новый файл дополняет их полным сквозным сценарием, не заменяет их точечное покрытие.

### 4.15.6. Финальная flag regression (все четыре комбинации)

Classic+Genetics off / Classic+Genetics on / Overhaul+Genetics off / Overhaul+Genetics on — оба Classic-билда байт-идентичны; ни одна V2-строка/символ (`RevealPanelV2`, `LabPanelV2`, `BotanicalBookPanel`, «Первый родитель» и т.д.) не присутствует ни в одном Classic-бандле; V2-данные (`nurseryTray`, растущие/зрелые гибриды, Reveal-метаданные, `pollen`/`geneticDust`/`pityCounter`/tutorial-флаги, включая новый `parentSpeciesIds`) переживают цикл V2→Legacy→V2 без потерь, Legacy-путь их не затирает; `ClassicApp.tsx`/`GardenScene.ts`/legacy `LabPanel.tsx`/`PlantPicker.tsx`/`AlbumPanel.tsx` не изменены; `SAVE_VERSION` остаётся `4`.

### 4.15.7. Критерии завершения Gate 1

Все из следующего одновременно: три коммита этого пакета созданы в заданном порядке и содержании; `git diff --check`/`tsc -b --noEmit` чисты; `oxlint` — честный факт baseline (0 новых warning); полный `vitest run` зелёный, включая новый `rarityCalibrationV2.test.ts`; rarity-симуляция — ровно 100 000 измеренных результатов, все шесть тиров внутри интервалов §4.15.4; все четыре production-сборки собраны с чистого кэша; Classic-сборки байт-идентичны; grep-изоляция подтверждена; ВСЕ существующие Playwright-сценарии зелёные; новый `test-e2e-genetics-v2-gate1.mjs` проходит целиком (все 37 пунктов); реальный save/reload проверен; mobile (360×800) и desktop (1366×768) — оба без проблем; `SAVE_VERSION=4`. Выполнение всех критериев переводит Gate 1 в статус **«готов к обзору владельцем»** — не «принят автоматически»: финальное решение о приёмке остаётся за владельцем.

---

## 5. Подтверждение

- **Новый файл**: `docs/GENETICS_GATE1_IMPLEMENTATION_CONTRACT.md` (этот документ) — создан в проходе 5, точечно исправлен в проходе 6 (§4.4, миграция `mutationId`), в проходе 7 (статус заголовка + §4.7, точное правило mutation-аллеля/RNG call order), в проходе 8 (статус заголовка + новый §4.8, persisted nursery lifecycle Slice 5), в проходе 9 (статус заголовка + новый §4.9, pollen economy Slice 6), в проходе 10 (статус заголовка + новый §4.10, recycling economy Slice 7, включая защитный фикс `pollenRewardV2` для неподдерживаемого species), в проходе 11 (статус заголовка + новый §4.11, Lab L2 «первый гибрид»/гейт Колокольника/минимальный микроскоп Slice 8), в проходе 12 (статус заголовка + новый §4.12, межвидовое V2-скрещивание Slice 9 — снятие `interspecies_locked` для поддерживаемых пар 1/2), в проходе 13 (статус заголовка + новый §4.13, объединённый пакет Slice 10-11 — прямые родители + legacy-species filtering), в проходе 14 (статус заголовка + новый §4.14, Reveal/естественное раскрытие/контекстный onboarding/Люми-подсказки/Ботаническая книга/демо-повтор Slice 12), в проходе 15 (статус заголовка + §4.14.7/§4.14.8/§4.14.9/§4.14.13 исправлены + новый §4.14.14 — fix-pass после отказа владельца от исходной Slice 12: Reveal перенесён на момент созревания, natural reveal исключает mutation-locus, экономика второго обучающего скрещивания восстановлена, гейтинг второго урока на реальном условии созревания+Reveal) и в проходе 16 (статус заголовка + §4.2 points/weights таблицы откалиброваны + §4.5.2/§4.5.4 пороги обновлены + новый §4.15 — финальный объединённый пакет Gate 1: carryover fix `parentSpeciesIds`, Slice 13 rarity calibration, Slice 14 финальный E2E), поверх `docs/GENETICS_TARGET_DELTA.md` (правки проходов 5-16) и без изменений `docs/GENETICS_ONBOARDING_SPEC.md`/`docs/GENETICS_CURRENT_STATE_AUDIT.md`.
- Python-скрипт симуляции (§4.5.5) существовал как источник чисел ДО прохода 16 — заменён РЕАЛЬНОЙ симуляцией в репозитории (§4.15.2, `apps/web/src/game/rarityCalibrationV2.ts`), не входящей в игровой runtime (test-support модуль, не импортируется production-кодом).
- **`main` не менялся.**
...
- **Статус на момент прохода 16 (финальный, этот проход)**: Slice 10-11 (§4.13) и Slice 12 включая fix-pass (§4.14/§4.14.14, коммит `bc26088e3822210c8fb11dd018414bf86291d385`) **приняты владельцем полностью**. Этот проход — финальный объединённый пакет Gate 1: carryover fix `parentSpeciesIds` (§4.15.1) + Slice 13 rarity calibration (§4.15.2/§4.15.3/§4.15.4) + Slice 14 финальный E2E/regression (§4.15.5/§4.15.6), ровно три коммита (docs → code+calibration → E2E). Критерии завершения — §4.15.7. **После этого пакета дальнейшая работа над Gate 1 не запланирована** — Slice 15/Lab L3-L4/`GeneLock`/новые виды/социальные механики/Supabase/Paddle/NFT/art-pass не начинаются без отдельного нового запроса владельца.
